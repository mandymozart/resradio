...
import { ABLY_REMOTE_CHANNEL, ABLY_ROTATION_CHANNEL, BREAKPOINT_MD, BREAKPOINT_XS, BROADCAST_MAX_ALLOWED_DURATION, BROADCAST_MIN_ALLOWED_DURATION, FUNCTIONS } from "../../../config";
...
const StudioPlayer = () => {
    ...
    const [rotationChannel] = useChannel(ABLY_ROTATION_CHANNEL, (message) => {
  ...
  `;
    ...
    useEffect(() => {
        if (broadcasts)
            loadBroadcast(data.allPlaylists.edges[0].node.broadcasts.filter(i => i.broadcast.audio?.includes(".mp3"))[0].broadcast)
                ...
                {broadcasts.map((node, index) => {
                    return (
                        <div key={index}
                            ...
                            <div className="duration">{node.broadcast.length}</div>
                        </div>
                    )
                }
                )}
